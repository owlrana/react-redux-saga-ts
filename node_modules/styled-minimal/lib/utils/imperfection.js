...
exports.Imperfection = exports.unique = exports.scale = exports.getOneOf = exports.getRandomBoolean = exports.getCurveSkew = exports.generateCircle = exports.convertToNumber = void 0;
...
function generateCircle(amount, randomize) {
    ...
    var variance = function () { return (amount && randomize ? amount + helpers_1.getRandomInt(-3, 3) : amount); };
...
function getCurveSkew(value, amount, direction) {
    if (amount === void 0) { amount = 0; }
...
exports.getCurveSkew = getCurveSkew;
function getRandomBoolean(ratio) {
    if (ratio === void 0) { ratio = 0.4; }
    return Math.random() >= ratio;
}
exports.getRandomBoolean = getRandomBoolean;
function getOneOf(inputA, inputB, ratio) {
    if (ratio === void 0) { ratio = 0.4; }
    return getRandomBoolean(ratio) ? inputA : inputB;
}
exports.getOneOf = getOneOf;
function scale(input, inputRange, masterRange) {
...
var Imperfection = /** @class */ (function () {
    ...
    Imperfection.prototype.getPath = function (mutation, complexity, asPercentage) {
        ...
        var points = helpers_1.limitNumber(Number(complexity), 0, 30);
        ...
        if (points) {
            for (var i = 0; i < points; i++) {
                ...
                output.push(this.getPoint(this.getRandomInt(helpers_1.limitNumber(base - 8, 0, this.xMax), helpers_1.limitNumber(base + 8, 0, this.xMax)), 'top', limit));
            }
        }
        ...
        if (points) {
            for (var i = points; i > 0; i--) {
                var base = divisions * (i - 1);
                output.push(this.getPoint(this.getRandomInt(helpers_1.limitNumber(base - 8, 0, this.xMax), helpers_1.limitNumber(base + 8, 0, this.xMax)), 'bottom', limit));
            }
        }
        ...
        if (asPercentage) {
            return output
                ...
                return helpers_1.getPercentage(p1, _this.xMax) + "% " + helpers_1.getPercentage(right, _this.yMax) + "%";
            })
                .join(', ');
        }
        return output.join(' ');
    ...
    Imperfection.prototype.getPoint = function (xPos, yPos, limit) {
        var left = helpers_1.isNumber(xPos) ? xPos : this.boundaries[xPos];
        var right = helpers_1.isNumber(yPos) ? yPos : this.boundaries[yPos];
        if (limit === 0) {
            return left + " " + right;
        }
        var p1 = this.getRandomInt(left === this.xMax ? left - 5 : left, helpers_1.limitNumber(left + limit, 0, this.getMax(left)));
        var p2 = this.getRandomInt(helpers_1.limitNumber(right - limit, 0, this.getMax(right)), helpers_1.limitNumber(right + limit, 0, this.getMax(right)));
        return p1 + " " + p2;
    };
    Imperfection.prototype.getRandomInt = function (min, max) {
        this.count++;
        return helpers_1.getRandomInt(min, max, unique("" + this.seed + this.count));
    };
    return Imperfection;
}());
exports.Imperfection = Imperfection;
